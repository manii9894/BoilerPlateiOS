...
class SnackBar: UIView, SnackBarAction, SnackBarPresentable {
    ...
    required public init(contextView: UIView, message: String, duration: Duration) {
        ...
        KeyboardHelper.shared.keyboardWillHide = { [weak self] in
            self?.dismiss()
        }
    }
    ...
    private func constraintSuperView(with view: UIView) {
        ...
        view.setupSubview(self) {
            $0.makeConstraints {
                ...
                $0.leading.equalTo(view.safeAreaLayoutGuide).offset(style.horizontalPadding)
                $0.trailing.equalTo(view.safeAreaLayoutGuide).offset(-style.horizontalPadding)
            }
        }
    ...
    public func show() {
        ...
        let keyboardHeight = KeyboardHelper.shared.keyboardHeight == 0 ? 0 : (KeyboardHelper.shared.keyboardHeight - style.verticalPadding)
        animation(with: -CGFloat(style.verticalPadding + keyboardHeight)) { _ in
